...
int main() {
    ...
    while (cin >> ch) {
        cin >> s1 >> s2;
        switch (ch) {
        case '+':  res = add(s1, s2); break;   //高精度加法
        case '-':  res = sub(s1, s2); break; //高精度减法
        case '*':  res = mul(s1, s2); break; //高精度乘法
        case '/':  res = div(s1, s2,1); break; //高精度除法, 返回商
        case 'm':  res = div(s1, s2,2); break; //高精度除法, 返回余数
        default:  break;
        }
        cout << res << endl;
    }
