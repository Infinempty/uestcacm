...
char cr[200];
int tt,top,rt,tot,n,m,ans,lastans,st[maxn*4];
struct rbt
{
    int v,sz,cnt;
    bool col;
...
void rotate(int o,bool c)
{
    int s1=t[o].ch[!c];
    t[o].ch[!c]=t[s1].ch[c];
    if (t[s1].ch[c]) t[t[s1].ch[c]].fa=o;
    t[s1].fa=t[o].fa;
    if(!t[o].fa) rt=s1;
    else t[t[o].fa].ch[get(o)]=s1;
    t[s1].ch[c]=o;
    t[o].fa=s1;
    pushup(o);
    pushup(s1);
...
int kth(int k) 
{
    ...
    for (;o;){
        tmp=t[t[o].ch[0]].sz;
        if(tmp+1<=k&&k<=tmp+t[o].cnt) break;
        else{
            if(k<=tmp) o=t[o].ch[0];
            else{
                k-=tmp+t[o].cnt;
...
{
    int o=rt;
    while(o && t[o].v!=v) o=t[o].ch[t[o].v<v];
...
int pre(int v) 
{
    ...
    int o=rt;
    for(;o;){
        if(t[o].v<v) res=t[o].v,o=t[o].ch[1];
        else o=t[o].ch[0];
    }
    return res;
}
