...
/*
最后两个位置是0或1 的4种集合
dp[i][j][k]= 长度为n，倒一位是j 倒二位是 k 的合法排列数量
...
00 和 11 对称 01和 10对称
dp[i][01+10]=dp[i-1][01+10]+dp[i-1][00+11]
dp[i][00+11]=dp[i-1][01+10]

ugly:
dp2[i]=pow(2,i)-(beautiful)
	  =2*pow(2,i-1)-(2*dp[i-1][1][0]+2*dp[i-1][0][1]+dp[i-1][1][1]+dp[i-1][0][0])
	  =2*dp2[i-1]+dp[i-1][1][1]+dp[i-1][0][0]
