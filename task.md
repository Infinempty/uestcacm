...
在istio植入Pod之后，istio会自动创建一个istio-init容器，这种容器是一种利用了Kubernetes特性，具有特权的特殊容器，在业务容器启动之前启动，该容器会根据istio的配置去修改Pod网络命名空间的iptables达到劫持流量的目的。istio在Pod网络命名空间iptables中新增的链：ISTIO_INBOUND，ISTIO_IN_REDIRECT，ISTIO_OUTPUT，ISTIO_REDIRECT，此外还并监听了两个端口，这里分别称为出站端口和入站端口。

对于入站流量，根据修改的iptables规则，会经过如下链路：
eth0->PREROUTING->ISTIO_INBOUND->ISTIO_IN_REDIRECT->ISTIO入站端口->SideCar容器->OUTPUT->ISTIO-OUTPUT->POSTROUTING->APP

对于出站流量，根据修改的iptables规则，会经过如下链路：
APP->OUTPUT->ISTIO-OUTPUT->ISTIO-REDIRECT->ISTIO出站端口->Sidecar容器->OUTPUT->ISTIO_OUTPUT->eth0

可以发现需要劫持的流量都经过了Sidecar容器，因此实现了对Pod出入站流量的劫持。
...
Service mesh的出现将容器和服务治理策略分离，新的Service上线时不需要单独开发SDK，也不需要单独部署网络配置，SDK只需要复用先前已有的，而网络配置则由控制平面统一下发，需要这种统一配置的主要原因在于云计算的特性：计算资源虚拟化之后，Pod和Service会大量出现，为每一个Service和Pod去开发SDK，进行配置实在是太过于耗时。

事实上在互联网大规模铺开之后，网络设备也面临着这样的问题，除了最基本的通信之外，如果想在网络之上装载一些其他功能，就需要进行大量的配置，例如运营商想要部署VPN，要配置MPLS、BFD、IGP、BGP、VPNV4、要绑定接口…且需要在每个PE上配置；当新增加一个PE时，还需要回去修改每个涉及到的PE。而且一旦这些功能影响到了数据链路层和传输层的复杂协议，很可能造成未知的破坏。

针对这些问题，研究者想到的一个方案是能否将支持用户APP运行的业务网络从交换机等硬件网络中分离出来？这样用户可以根据自己的需求自定义网络模型而又不至于影响到网络硬件的正常运行，基于这个想法，网络工程师们提出了SDN(Software defined network)这个概念，这个概念将网络分为两层：业务网络Overlay Network和承载网络Underlay Network。

业务网络具体承载用户的需求，设备之间的互联通过VXLAN之类的网络虚拟化技术实现，对于业务网络中的网络应用，他们传出的数据包经VXLAN就仿佛发送到了巨大的二层交换机，每一个网络设备都身处交换机之中，可以快速进行通信，业务网络中的具体配置由一个控制平面下发，控制平面会收集整个业务网络的网络信息，为SDN网络下达正确的配置。

承载网络则具体承担通信的功能，业务网络对于他们来说是透明的，承载网络不需要知道业务网络对原始数据包做了什么处理（例如存在一个代理，进行了多少次VPN，原始数据包上封装了几层协议等等），只需要根据IP将数据包发送至合适的位置即可。



### (2) srv6

（SRV6示意图，展现出低延迟，低丢包不同特性什么的）
...
多个应用程序之间需要同一级别的服务也需要使用策略路由。问题是，这些应用程序获取带宽时，通信类带宽的分配是满了的吗?如何让网络告诉应用程序，在没有带宽的情况下还能知道所需的流量类别?这就是为什么我们需要软件定义网络(SDN)的原因，因为它让应用程序或策略管理器来进行流量转发。OpenFlow技术有能力使其应用政策变得动态化，现在，一个相对较新的技术提供了另一种方法：分段路由(Segment Routing)。SR是SDN的一种，类似于其他SDN技术，SR也拥有一个控制平面（控制器），根据用户需要，控制器能够收集网络拓扑信息并规划一个数据包具体的路由转发路径，从而为用户提供不同网络服务。

SRV6是SR与IPV6结合的产物，在IPV4上运行的SR需要MPLS数据平面的支持，需要整个网络的设备都支持才能够运作，扩展性不强。然而SRV6的设计只需要设备支持IPV6即可转发，扩展性良好。目前IETF已经发表声明，停止为IPv4制定更新的标准，在IPV6逐渐普及的今天，SRV6拥有比SR-MPLS更好的发展前景。
### (3) ipv6背景下广域多集群服务互联技术

（未来架构图，EXCEL）

回顾Kubernetes对Pod IP地址的要求是什么？其一是扁平化，所有Pod拥有在集群内唯一的IP地址；其二是去NAT化，Pod之间不经NAT就可以互相通信。

对于异地集群，这两个要求在IPV4时代就很难实现了，尽管有Flannel和VXLAN技术保证了Pod IP的扁平化，但是在实际上传输过程中，数据包其实已经不知道在运营商网络内被NAT了多少次了。并且当一个项目拆分成微服务时，服务之间的调用变成了网络通信，程序员一个特有的错觉就是网络是可靠的，事实上只有内网勉强算的上是可靠的，异地的，经过运营商网络的通信就很难说是可靠的了，可靠性差，延迟高，带宽小，Sidecar容器运行在用户态中对性能造成的损失，这些因素都对异地微服务的质量造成了挑战。

在SR+IPV6时代，这些问题得到了一定程度上的缓解。首先，由于IPV6原生支持IP扁平化，我们可以直接舍弃Kubernetes中的Flannel插件，让Service直接运行在IPV6环境下，这样不要说集群内了，就算是集群外，Pod的IP也是不会重复的。其次由于SRV6技术的出现，微服务可以根据自己的需要规划路由路径，一定程度上缓解了异地服务调用存在的问题。

不可否认的是，Service mesh和SDN本身十分相似，他们都拥有控制平面，并且实现了业务逻辑和治理策略的分离，在未来，由于IPV6的全面展开，我认为Service mesh会与SRV6技术结合，不仅在软件技术方面会有持续发展，也会根据具体的业务需要下沉到硬件，为Service mesh推出专门的硬件以及控制器，在其中集成以SRV6为代表的SDN技术，推动广域多集群服务互联技术全面发展。
